...
export const getUsers = async (page, perPage, { search = '', status = 'all' } = {}) => {
    const skip = (page - 1) * perPage;
    const filter = adminRepository.buildUserFilter({ search, status });
    const [total, users] = await Promise.all([
        adminRepository.countUsers(filter),
        adminRepository.findUsers(filter, { skip, limit: perPage })
    ]);
