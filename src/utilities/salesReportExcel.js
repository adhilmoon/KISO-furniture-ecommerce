...
export const streamSalesReportExcel = async (res, { label, summary, breakdown, orders }) => {
    ...
    for (const [lbl, value] of summaryRows) {
        sheet.getCell(`A${rowIdx}`).value = lbl;
    ...
    // ── Orders sheet ───────────────────────────────────────────────────────────
    // Matches PDF columns: Order ID | Date | Customer | Payment Method | Status
    //                      Subtotal | Discount | Grand Total | Coupon Code
    // No Tax column (KISO does not track tax separately)
    if (orders && orders.length > 0) {
        ...
        ordersSheet.columns = [
            ...
            { header: 'Discount',       key: 'couponDiscount', width: 14 },
